/**
 * EconomicTimesAdapter — Tier-1 RSS-based adapter for The Economic Times.
 *
 * The Economic Times publishes a public RSS feed which is used as the primary
 * fetch mechanism (Req 1.4: prefer official feed over scraping).
 * The ScraplingClient is used only for the health check's sidecar reachability
 * probe; article fetching is fully RSS-driven.
 *
 ...
 *
 *   NEWS_SOURCE_ECONOMICTIMES_BASE_URL  — RSS feed URL
 *                                         Default: https://economictimes.indiatimes.com/rssfeedsdefault.cms
 *   NEWS_SOURCE_ECONOMICTIMES_RPM       — requests-per-minute limit (optional, default 10)
...
const logger = pino({ name: 'economic-times-adapter' });
...
/**
 * Tier-1 adapter for The Economic Times news via their public RSS feed.
 *
 * `healthCheckImpl()` verifies both the Scrapling sidecar (via
 * ScraplingClient.healthCheck()) and the Economic Times RSS feed itself
 * (HTTP 200 check).
 *
 ...
 * using the same SHA-256 hash logic as ReutersAdapter and MoneycontrolAdapter.
 */
export class EconomicTimesAdapter extends AbstractNewsSourceAdapter {
  // ------------------------------------------------------------------
  ...
  constructor(scraplingClient?: ScraplingClient) {
    ...
    this.feedUrl =
      process.env['NEWS_SOURCE_ECONOMICTIMES_BASE_URL'] ?? DEFAULT_FEED_URL;

    this._requestsPerMinute = Math.max(
      1,
      parseInt(process.env['NEWS_SOURCE_ECONOMICTIMES_RPM'] ?? '10', 10) || 10,
    );
    ...
    this.http = axios.create({
      ...
      headers: {
        ...
        'User-Agent': `SentinelPulse-EconomicTimesAdapter/${this.adapterVersion}`,
      },
    });
   ...
   *   2. Probing the Economic Times RSS feed URL for an HTTP 200 response.
   *
  ...
  protected async healthCheckImpl(): Promise<HealthStatus> {
    ...
    try {
      ...
      logger.warn(
        ...
        'EconomicTimes: Scrapling sidecar health check threw',
      );
    }
    ...
    if (!sidecarHealthy) {
      ...
      logger.warn(
        ...
        'EconomicTimes health check failed: Scrapling sidecar is not healthy',
      );
      return {
        healthy: false,
    ...
    try {
      ...
      return {
        healthy,
        message: healthy
          ? 'Economic Times RSS feed is reachable'
          : `Economic Times RSS feed returned HTTP ${res.status}`,
        checkedAt,
      ...
      const message =
        err instanceof Error
          ? err.message
          : 'Unknown error during Economic Times health check';

      logger.warn(
        { err, sourceId: this.sourceId },
        'Economic Times RSS feed health check failed',
      );
  ...
  /**
   * Fetches the Economic Times RSS feed and returns parsed articles as
   * RawArticle[].
   *
  ...
  protected async fetchLatestImpl(options: FetchOptions): Promise<RawArticle[]> {
    ...
    logger.debug(
      ...
      'Parsed RSS items from Economic Times feed',
    );
  ...
  /**
   * Fetches historical articles from the Economic Times RSS feed.
   *
  ...
  /**
   * Maps an Economic Times RawArticle into the canonical NormalizedArticle.
   *
    ...
    if (raw.publishedAt) {
      ...
      if (isNaN(parsed.getTime())) {
        logger.warn(
          ...
          'EconomicTimes: could not parse publishedAt — inferring current UTC time',
        );
